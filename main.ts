...
function getOptions(): Options {
  return {
    ...
    forceVersion:
      (core.getInput("force-version") || "").toLowerCase() === "true",
...
function getWin32Url(version: string, options: Options): string | null {
  ...
  const prefix = "LLVM-";
  const suffix =
    compareVersions(version, "3.7.0") >= 0 ? "-win64.exe" : "-win32.exe";
  if (compareVersions(version, "9.0.1") >= 0) {
    return getGitHubUrl(version, prefix, suffix);
  } else {
    return getReleaseUrl(version, prefix, suffix);
...
export function getSpecificVersionAndUrl(
  platform: string,
  options: Options
): [string, string] {
  ...
  if (!VERSIONS.has(options.version)) {
    throw new Error(
      `Unsupported target! (platform='${platform}', version='${options.version}')`
    );
  }
...
async function install(options: Options): Promise<void> {
  ...
  core.info(
    `Installed LLVM and Clang ${options.version} (${specificVersion})!`
  );
